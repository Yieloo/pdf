...
class createPDF {
	...
	function run() {
		...
		$pdf->SetCreator("ooPartners Solutions");
...
class HTML2PDF extends FPDF
{
	...
	function HTML2PDF($orientation='P',$unit='mm',$format='A4',$_title='',$_url='',$_debug=false)
	{
		...
		$this->fontlist=array("Times","Courier","Helvetica","Arial","Symbol","ZapfDingbats","Comic");
		$this->AddFont('Comic');
		$this->AddFont('Comic','B','comicbd.php');
		$this->AddFont('Comic','I','comici.php');
		$this->AddFont('Comic','BI','comicz.php');
		$this->AddFont('Tahoma');
		$this->AddFont('Tahoma','B','tahomabd.php');
		$this->AddFont('Tahoma','I','tahomai.php');
		$this->AddFont('Tahoma','BI','tahomaz.php');
		$this->AddFont('Verdana');
		$this->AddFont('Verdana','B','verdanab.php');
		$this->AddFont('Verdana','I','verdanai.php');
		$this->AddFont('Verdana','BI','verdanaz.php');
	...
	function WriteHTML($html,$bi=true)
	{
		...
		$this->bi=$bi;
		if ($bi)
			$html=strip_tags($html,"<a><img><p><br><font><tr><blockquote><h1><h2><h3><h4><pre><red><blue><ul><li><hr><div><b><i><u><strong><em>"); 
		else
			$html=strip_tags($html,"<a><img><p><br><font><tr><blockquote><h1><h2><h3><h4><pre><red><blue><ul><li><hr><div>"); 
	...
	function OpenTag($tag,$attr)
	{
		...
		switch($tag){
			...
			case 'LI':
				//$this->Ln();
				//$this->SetTextColor(243,122,33);// orange comme le logo
				//$this->Write(5,'     � ');
				//$this->mySetTextColor(-1);
				$this->SetX($this->GetX()+5);
				$this->Image("$images/puce.png", $this->GetX(), $this->GetY()+1.75);
			...
			case 'FONT':
				...
				if (isset($attr['SIZE']) && $attr['SIZE']!='') {
					switch($attr['SIZE']){
						case 1:$this->SetFontSize(8);break;
						case 2:$this->SetFontSize(10);break;
						case 3:$this->SetFontSize(12);break;
						case 4:$this->SetFontSize(14);break;
						case 5:$this->SetFontSize(18);break;
						case 6:$this->SetFontSize(24);break;
						case 7:$this->SetFontSize(36);break;
						default : $this->SetFontSize(12);	
					}					
					$this->issetsize=true;
				}
				break;
	...
	{//si le tag commence par un /, on le supprime
		if (substr(trim($tag),0,1)=='/')
			$tag=substr(trim($tag),1);
		//Closing tag
